...
class SectionEditorAction extends Action {
            ...
            if (!$email->isEnabled() || ($send && !$email->hasErrors())) {
                ...
                if (!$request->getUserVar('continued')) {
                    ...
                    $paramArray = array(
                        ...
                        /*MUNIPRESS*/
                        'editorialContactNameCPSY' => $user->getFullName(),
                        /*************/
    ...
    function remindReviewer($sectionEditorSubmission, $reviewId, $send, $request) {
        ...
        if ($send && !$email->hasErrors()) {
            ...
            $email->displayEditForm(
                    $request->url(null, null, 'remindReviewer', 'send'), array(
    ...
    function setDueDate($articleId, $reviewId, $dueDate, $numWeeks, $logEntry, $request) {
        ...
        if ($reviewAssignment->getSubmissionId() == $articleId && !HookRegistry::call('SectionEditorAction::setDueDate', array(&$reviewAssignment, &$reviewer, &$dueDate, &$numWeeks))) {
            ...
            if ($logEntry) {
                ...
                import('classes.article.log.ArticleEventLogEntry');
                ArticleLog::logEvent(
                        $request, $sectionEditorSubmission, ARTICLE_LOG_REVIEW_SET_DUE_DATE, 'log.review.reviewDueDateSet', array(
                    'reviewerName' => $reviewer->getFullName(),
    ...
    function clearReviewForm($sectionEditorSubmission, $reviewId) {
        ...
        if (HookRegistry::call('SectionEditorAction::clearReviewForm', array(&$sectionEditorSubmission, &$reviewAssignment, &$reviewId)))
            return $reviewId;
    ...
    function addReviewForm($sectionEditorSubmission, $reviewId, $reviewFormId) {
        ...
        if (HookRegistry::call('SectionEditorAction::addReviewForm', array(&$sectionEditorSubmission, &$reviewAssignment, &$reviewId, &$reviewFormId)))
            return $reviewFormId;
    ...
    function viewReviewFormResponsePrint($sectionEditorSubmission, $reviewId) {
        ...
        if (HookRegistry::call('SectionEditorAction::viewReviewFormResponsePrint', array(&$sectionEditorSubmission, &$reviewAssignment, &$reviewId)))
            return $reviewId;
    ...
    function completeCopyedit($sectionEditorSubmission, $request) {
        ...
        if (HookRegistry::call('SectionEditorAction::completeCopyedit', array(&$sectionEditorSubmission)))
            return;
    ...
    function restoreToQueue($sectionEditorSubmission, $request) {
        if (HookRegistry::call('SectionEditorAction::restoreToQueue', array(&$sectionEditorSubmission)))
            return;
    ...
    function updateCommentsStatus($submission, $commentsStatus) {
        if (HookRegistry::call('SectionEditorAction::updateCommentsStatus', array(&$submission, &$commentsStatus)))
            return;
    ...
    function deleteArticleImage($submission, $fileId, $revision) {
        ...
        if (HookRegistry::call('SectionEditorAction::deleteArticleImage', array(&$submission, &$fileId, &$revision)))
            return;
    ...
    function removeSubmissionNote($articleId, $noteId, $fileId) {
        if (HookRegistry::call('SectionEditorAction::removeSubmissionNote', array(&$articleId, &$noteId, &$fileId)))
            return;
    ...
    function postPeerReviewComment(&$article, $reviewId, $emailComment, $request) {
        if (HookRegistry::call('SectionEditorAction::postPeerReviewComment', array(&$article, &$reviewId, &$emailComment)))
            return;
        ...
        if ($commentForm->validate()) {
            ...
            foreach ($notificationUsers as $userRole) {
                $notificationManager->createNotification(
                        $request, $userRole['id'], NOTIFICATION_TYPE_REVIEWER_COMMENT, $article->getJournalId(), ASSOC_TYPE_ARTICLE, $article->getId()
                );
            }
    ...
    function viewEditorDecisionComments($article) {
        if (HookRegistry::call('SectionEditorAction::viewEditorDecisionComments', array(&$article)))
            return;
    ...
    function postEditorDecisionComment($article, $emailComment, $request) {
        ...
        if ($commentForm->validate()) {
            ...
            foreach ($notificationUsers as $userRole) {
                $notificationManager->createNotification(
                        $request, $userRole['id'], NOTIFICATION_TYPE_EDITOR_DECISION_COMMENT, $article->getJournalId(), ASSOC_TYPE_ARTICLE, $article->getId()
                );
            }
    ...
    function emailEditorDecisionComment($sectionEditorSubmission, $send, $request) {
        ...
        $email = new ArticleMailTemplate(
                $sectionEditorSubmission, isset($decisionTemplateMap[$decisionConst]) ? $decisionTemplateMap[$decisionConst] : null
        );
                ...
                foreach ($reviewAssignments as $reviewAssignment) {
                    ...
                    if ($reviewAssignment->getDateCompleted() != null && !$reviewAssignment->getCancelled()) {
                        ...
                        if ($articleComments) {
                            ...
                            /* MUNIPRESS */
                            if ($journal->getId() == 67) {
                                $body .= __('submission.comments.importPeerReviews.reviewerLetter', array('reviewerLetter' => $reviewCounter++)) . "\n";
                            } else {
        ...
        function postCopyeditComment($article, $emailComment, $request) {
            ...
            if ($commentForm->validate()) {
                ...
                foreach ($notificationUsers as $userRole) {
                    $notificationManager->createNotification(
                            $request, $userRole['id'], NOTIFICATION_TYPE_COPYEDIT_COMMENT, $article->getJournalId(), ASSOC_TYPE_ARTICLE, $article->getId()
                    );
                }
        ...
        function postLayoutComment($article, $emailComment, $request) {
            if (HookRegistry::call('SectionEditorAction::postLayoutComment', array(&$article, &$emailComment)))
                return;
            ...
            if ($commentForm->validate()) {
                ...
                foreach ($notificationUsers as $userRole) {
                    $notificationManager->createNotification(
                            $request, $userRole['id'], NOTIFICATION_TYPE_LAYOUT_COMMENT, $article->getJournalId(), ASSOC_TYPE_ARTICLE, $article->getId()
                    );
                }
        ...
        function viewProofreadComments($article) {
            if (HookRegistry::call('SectionEditorAction::viewProofreadComments', array(&$article)))
                return;
        ...
        function postProofreadComment($article, $emailComment, $request) {
            ...
            if ($commentForm->validate()) {
                ...
                foreach ($notificationUsers as $userRole) {
                    $notificationManager->createNotification(
                            $request, $userRole['id'], NOTIFICATION_TYPE_PROOFREAD_COMMENT, $article->getJournalId(), ASSOC_TYPE_ARTICLE, $article->getId()
                    );
                }
        ...
        function confirmReviewForReviewer($reviewId, $accept, $request) {
            ...
            if (HookRegistry::call('SectionEditorAction::acceptReviewForReviewer', array(&$reviewAssignment, &$reviewer, &$accept)))
                return;
