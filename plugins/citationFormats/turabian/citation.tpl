...
{foreach from=$authors item=author name=authors key=i}
	{assign var=firstName value=$author->getFirstName()}
        {if $journal->getSetting('allowMedailonCitations')}
            <a href="{url page="about" op="bioAuthor" path=$author->getId()}">{$author->getLastName()|escape}, {$firstName|escape}</a>{if $i==$authorCount-2}, {translate key="rt.context.and"} {elseif $i<$authorCount-1}, {else}.{/if}
        {else}
