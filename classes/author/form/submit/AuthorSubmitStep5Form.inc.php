...
class AuthorSubmitStep5Form extends AuthorSubmitForm {
    ...
    function checkWaiverReason() {
        if ($this->request->getUserVar('qualifyForWaiver') == false)
            return true;
        else
            return ($this->request->getUserVar('commentsToEditor') != '');
    }
    ...
    function execute() {
        ...
        if ($mail->isEnabled()) {
            ...
            if ($journal->getSetting('copySubmissionAckPrimaryContact')) {
                $mail->addBcc(
                        $journal->getSetting('contactEmail'), $journal->getSetting('contactName')
                );
            }
...
//			}
        ...
        /* MUNIPRESS */
        if (sizeof($sectionEditors) > 0) {
            foreach ($sectionEditors as $sectionEditorEntry) {
                $mailSectionEditor = new ArticleMailTemplate($article, 'EDITOR_ASSIGN', null, null, null, false);
                $mailSectionEditor->setReplyTo(null);
                if ($mailSectionEditor->isEnabled()) {
                    $sectionEditor = & $sectionEditorEntry['user'];
                    $mailSectionEditor->addRecipient($sectionEditor->getEmail(), $sectionEditor->getFullName());

                    $mailSectionEditor->assignParams(array(
                        'editorialContactName' => $sectionEditor->getFullName(),
                        'editorUsername' => $sectionEditor->getUsername(),
                        'editorialContactSignature' => $journal->getSetting('contactName') . "\n" . $journal->getLocalizedTitle(),
                        'submissionUrl' => $this->request->url(null, 'editor', 'submissionReview', $article->getId())
                    ));
                    $mailSectionEditor->send($this->request);
                }
                unset($sectionEditor);
            }
        }
