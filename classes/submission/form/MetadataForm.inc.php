...
class MetadataForm extends Form {
    ...
    function MetadataForm($article, $journal) {
        ...
        if ($this->canEdit) {
            ...
            parent::Form(
                    'submission/metadata/metadataEdit.tpl', true, $article->getLocale(), array_flip(array_intersect(
                                    array_flip(AppLocale::getAllLocales()), $supportedSubmissionLocales
                    ))
            );
        ...
        if (isset($this->article)) {
            ...
            for ($i = 0, $count = count($authors); $i < $count; $i++) {
                array_push(
                    ...
                    'journalAuthorId' => (int)$authors[$i]->getData('journalAuthorId'),
                    )
                );
    ...
    function execute(&$request) {
        ...
        for ($i = 0, $count = count($authors); $i < $count; $i++) {
            ...
            if ($author != null) {
                ...
                $author->setData('journalAuthorId', (int)$authors[$i]['journalAuthorId']);
