...
<script type="text/javascript">
    {literal}
        $(document).ready(function () {
            setupTableDND("#dragTable", "moveGroup");
        });
    {/literal}
</script>
<br />
...
{if count($formLocales) > 1}
	...
	<tr valign="top">
		...
		<td width="80%" class="value">
			{url|assign:"editorialTeamFormUrl" op="setEditorialTeamDescription" escape=false}
			{form_language_chooser form="editorialTeamForm" url=$editorialTeamFormUrl}
...
{/if}
    ...
    <div id="editorialTeamDescription">
        <h3>{translate key="manager.groups.editorialTeamDescription"}</h3>
        <p>
            <textarea name="editorialTeamDescription[{$formLocale|escape}]" id="editorialTeamDescription" rows="8" cols="60" class="textArea">{$editorialTeamDescription[$formLocale]|escape}</textarea>
        </p>
    </div>
    <input type="submit" value="{translate key="common.record"}" class="button defaultButton"/>
</form>*}
