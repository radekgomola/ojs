 ...
 *
 * $Id: index.tpl,v 1.14 2008/06/11 18:55:19 asmecher Exp $
 *}
{assign var="pageTitle" value="plugins.generic.publicfolderbrowser.displayName"}
{include file="common/header.tpl"}
{translate key="plugins.generic.publicfolderbrowser.descriptionOnSite"}
{assign var=displayDir value="/$currentDir"}
<h3>{translate key="manager.files.indexOfDir" dir=$displayDir|escape}</h3>
{if $currentDir}
<p><a href="{url op="publicFolder" path=$parentDir|explode:"/"}" class="action">&lt; {translate key="manager.files.parentDir"}</a></p>
{/if}
...
<table width="100%" class="listing">
	...
	<tr valign="top">
		...
		<td><a href="{url op="publicFolder" path=$filePath|explode:"/"}" title="{$file.name}">{$file.name|truncate:30}</a></td>
		...
		<td align="right" class="nowrap">
			{if !$file.isDir}
        <a href="{$publicFilesDir}/{$filePath}" class="action" target="_blank">LINK</a>&nbsp;|
				<a href="{url op="publicFolder" path=$filePath|explode:"/" download=1}" class="action">{translate key="common.download"}</a>&nbsp;|  
			{/if}
			<a href="{url op="publicFileDelete" path=$filePath|explode:"/"}" onclick="return confirm('{translate|escape:"jsparam" key="manager.files.confirmDelete"}')" class="action">{translate key="common.delete"}</a>
		</td>
	</tr>
	<tr>
...
<form method="post" action="{url op="publicFileUpload" path=$currentDir|explode:"/"}" enctype="multipart/form-data">
...
<form method="post" action="{url op="publicFileMakeDir" path=$currentDir|explode:"/"}" enctype="multipart/form-data">
