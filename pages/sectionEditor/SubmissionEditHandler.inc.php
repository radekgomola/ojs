...
class SubmissionEditHandler extends SectionEditorHandler {
    ...
    function _getFrom($default = 'submissionEditing') {
        ...
        if (!in_array($from, array('submission', 'submissionEditing')))
            return $default;
        return $from;
    }
    ...
    function suggestUsername($args, &$request) {
        ...
        $suggestion = Validation::suggestUsername(
                        $request->getUserVar('firstName'), $request->getUserVar('lastName')
        );
        echo $suggestion;
    }
        ...
        if ($submitForm->validate()) {
            ...
            foreach ($notificationUsers as $userRole) {
                $notificationManager->createNotification(
                        $request, $userRole['id'], NOTIFICATION_TYPE_SUPP_FILE_MODIFIED, $article->getJournalId(), ASSOC_TYPE_ARTICLE, $article->getId()
                );
            }
    ...
    function saveGalley($args, $request) {
        ...
        if ($submitForm->validate()) {
            ...
            foreach ($notificationUsers as $userRole) {
                $notificationManager->createNotification(
                        $request, $userRole['id'], NOTIFICATION_TYPE_GALLEY_MODIFIED, $article->getJournalId(), ASSOC_TYPE_ARTICLE, $article->getId()
                );
            }
    ...
    function downloadFile($args, &$request) {
        ...
        $suffix = $request->getUserVar('suffix');

        $this->validate($articleId);
        if (!SectionEditorAction::downloadFile($articleId, $fileId, $revision, $suffix)) {
            $request->redirect(null, null, 'submission', $articleId);
        }
    }
    ...
    /**
     * Download a zip of supplementary files.
     * @param $args submission ID ($)
     * @param $request PKPRequest
     */
    function downloadSuppZip($args, $request) {
        ...
        $articleFiles = & $submission->getSuppFiles();
        foreach ($articleFiles as $articleFile) {
            if (isset($articleFile)) {
                $fileType = $articleFile->getFileType();
                $filePath = $articleFileManager->filesDir . 'supp/' . $articleFile->getFileName();
                
                if($suffix){
                    $info = pathinfo(basename($filePath));
                    $nazev = $info["filename"]."-".$articleFile->getOriginalFileName();
                    $filePaths[$filePath] = $nazev;
                }else{
                    $filePaths[$filePath] = $articleFile->getFileName();
                }                
            }
        }
...
//        exit();
        ...
        $archivePath = $fileArchive->createSuppZip($filePaths);
        if (file_exists($archivePath)) {
            $fileManager = new FileManager();
            if ($fileArchive->zipFunctional()) {
                $fileManager->downloadFile($archivePath, $fileManager->_fileMimeType($archivePath), false, 'SupplementaryFiles.zip');
            } else {
                $fileManager->downloadFile($archivePath, $fileManager->_fileMimeType($archivePath), false, 'SupplementaryFiles.tar.gz');
            }
    ...
    function waiveSubmissionFee($args, $request) {
        ...
        $queuedPayment = & $paymentManager->createQueuedPayment(
                        $journal->getId(), PAYMENT_TYPE_SUBMISSION, $markAsPaid ? $submission->getUserId() : $user->getId(), $articleId, $markAsPaid ? $journal->getSetting('submissionFee') : 0, $markAsPaid ? $journal->getSetting('currency') : ''
        );
    ...
    function waiveFastTrackFee($args, &$request) {
        ...
        $queuedPayment = & $paymentManager->createQueuedPayment(
                        $journal->getId(), PAYMENT_TYPE_FASTTRACK, $markAsPaid ? $submission->getUserId() : $user->getId(), $articleId, $markAsPaid ? $journal->getSetting('fastTrackFee') : 0, $markAsPaid ? $journal->getSetting('currency') : ''
        );
    ...
    function waivePublicationFee($args, $request) {
        ...
        $queuedPayment = & $paymentManager->createQueuedPayment(
                        $journal->getId(), PAYMENT_TYPE_PUBLICATION, $markAsPaid ? $submission->getUserId() : $user->getId(), $articleId, $markAsPaid ? $journal->getSetting('publicationFee') : 0, $markAsPaid ? $journal->getSetting('currency') : ''
        );
